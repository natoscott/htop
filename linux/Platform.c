...
static Htop_Reaction Platform_actionSetIOPriority(State* st) {
   ...
   if (set) {
      ...
      bool ok = MainPanel_foreachRow(st->mainPanel, LinuxProcess_setIOPriority, (Arg) { .i = ioprio2 }, NULL);
      if (!ok) {
         beep();
      }
   }
...
static bool Platform_changeAutogroupPriority(MainPanel* panel, int delta) {
   ...
   bool anyTagged;
   bool ok = MainPanel_foreachRow(panel, LinuxProcess_changeAutogroupPriorityBy, (Arg) { .i = delta }, &anyTagged);
   if (!ok)
      beep();
   return anyTagged;
}
