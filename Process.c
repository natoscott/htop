...
void Process_writeField(const Row* super, RichString* str, RowField field) {
   char buffer[256];
   ...
   case ELAPSED: {
      const uint64_t rt = super->host->realtimeMs;
   ...
   case PERCENT_NORM_CPU: {
      float cpuPercentage = this->percent_cpu / super->host->activeCPUs;
...
bool Process_isVisible(const Row* super, const Table* table) {
   const Process* this = (const Process*) super;
   const Settings* settings = table->host->settings;

   if (settings->hideUserlandThreads)
      return ! Process_isThread(this);
   return true;
}

/* Test whether display must filter out this process (various mechanisms) */
bool Process_matchesFilter(const Row* super, const Table* table) {
   const Process* this = (const Process*) super;
...
bool Process_setPriority(Row* super, int priority) {
   Process* this = (Process*) super;
...
bool Process_changePriorityBy(Row* super, Arg delta) {
   Process* this = (Process*) super;
   return Process_setPriority(super, this->nice + delta.i);
}

bool Process_sendSignal(Row* super, Arg sgn) {
   Process* this = (Process*) super;
