...
static void addDynamicScreen(ATTR_UNUSED ht_key_t key, void* value, void* userdata) {
   ...
   char* const name = screen->heading ? screen->heading : screen->name;

fprintf(stderr, "Adding dynamic screen %s to Screens panel\n", name);
...
void ActiveScreensPanel_fill(ActiveScreensPanel* this, DynamicScreen* ds) {
   ...
   /* TODO walk the ScreenSettings array? need to match those to given screen ds */
   if (ds == NULL) {
      /* special case of built-in (processes) not dynamic - add e.g. Main an I/O */
fprintf(stderr, "ActiveScreensPanel_fill: processes\n");
   }
   else {
      /* find screens using this set */
fprintf(stderr, "ActiveScreensPanel_fill: dynamic\n");
   }
}
...
static ActiveScreenListItem* ActiveScreenListItem_new(const char* value, ScreenSettings* ss) {
...
static void addNewScreen(Panel* super) {
   ...
   const char* name = "New";
   // TODO: for 'processes' this is fine - what about DynamicScreens?...
...
static HandlerResult ActiveScreensPanel_eventHandlerNormal(Panel* super, int ch) {
   ...
   if (newFocus && oldFocus != newFocus) {
      ColumnsPanel_fill(this->activeColumns, newFocus->ss, this->settings->dynamicColumns);
      if (newFocus->ss->dynamic) {
         char* currentScreen = newFocus->ss->dynamic;
...
ActiveScreensPanel* ActiveScreensPanel_new(Settings* settings) {
   ...
   super->cursorOn = false;
   this->cursor = 0;
   Panel_setHeader(super, "Active Screens");
