...
IOPriority LinuxProcess_updateIOPriority(Row* super) {
   IOPriority ioprio = 0;
// Other OSes masquerading as Linux (NetBSD?) don't have this syscall
#ifdef SYS_ioprio_get
   ioprio = syscall(SYS_ioprio_get, IOPRIO_WHO_PROCESS, super->id);
#endif
   LinuxProcess* this = (LinuxProcess*) super;
   this->ioPriority = ioprio;
   return ioprio;
}

bool LinuxProcess_setIOPriority(Row* super, Arg ioprio) {
// Other OSes masquerading as Linux (NetBSD?) don't have this syscall
#ifdef SYS_ioprio_set
   syscall(SYS_ioprio_set, IOPRIO_WHO_PROCESS, super->id, ioprio.i);
#endif
   return (LinuxProcess_updateIOPriority(super) == ioprio.i);
}
...
bool LinuxProcess_changeAutogroupPriorityBy(Row* super, Arg delta) {
   char buffer[256];
   xSnprintf(buffer, sizeof(buffer), PROCDIR "/%d/autogroup", super->id);
...
static void LinuxProcess_writeField(const Row* super, RichString* str, ProcessField field) {
